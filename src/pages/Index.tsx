...
const Index = () => {
  ...
  const detectMemoryQuery = (text: string): boolean => {
    const memoryKeywords = ["remember", "remind me", "what did we talk about", "what did I tell you", "previous conversation", "last time", "you told me", "recall", "memory", "we discussed", "you mentioned", "we talked about", "I asked you", "fetch memory", "search memory"];
  ...
  const detectServiceRequest = (text: string) => {
    const lowerText = text.toLowerCase();
    if (lowerText.includes("email") || lowerText.includes("gmail") || lowerText.includes("inbox") || lowerText.includes("message") || lowerText.includes("mail")) {
      return "gmail";
    }
    if (lowerText.includes("calendar") || lowerText.includes("schedule") || lowerText.includes("meeting") || lowerText.includes("event") || lowerText.includes("appointment")) {
      return "calendar";
    }
    if (lowerText.includes("drive") || lowerText.includes("file") || lowerText.includes("document") || lowerText.includes("upload") || lowerText.includes("folder")) {
      return "drive";
    }
    return null;
  };
  ...
  const handleSendMessage = async (text: string) => {
    ...
    setMessages(prevMessages => prevMessages.filter(msg => msg.id !== loadingMessageId).concat({
  ...
  return <div className="flex flex-col h-screen bg-gradient-to-b from-pink-50 via-purple-50 to-indigo-50 overscroll-none">
